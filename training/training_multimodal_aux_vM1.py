...
from tools import tools_aux_loss_vM1 as tools
...
def train_epochs(args, model, optimizer, params, dicts, struc_feats, struc_labels):
    ...
    for epoch in range(args.n_epochs):
        ...
        metrics_all = one_epoch(model, optimizer, epoch, args.n_epochs, args.batch_size, args.data_path, test_only, dicts, model_dir, 
                                ...
                                opt_thresh, args.struc_aux_loss_wt)
        end = time.time()
...
def one_epoch(model, optimizer, epoch, n_epochs, batch_size, data_path, testing_only, dicts, model_dir, gpu, quiet, 
              ...
              test_frac, opt_thresh, struc_aux_loss_wt):
    """
        Basically a wrapper to do a training epoch and test on dev
    """
    if not testing_only:        
        
        losses = train(model, optimizer, epoch, batch_size, data_path, struc_feats_train_std, struc_labels_train, gpu, dicts, quiet, struc_aux_loss_wt, train_frac)
...
def train(model, optimizer, epoch, batch_size, data_path, struc_feats, struc_labels, gpu, dicts, quiet, struc_aux_loss_wt, train_frac=1): ### struc feats = full structured sparse matrix
    """
        Training loop.
    ...
    for batch_idx, tup in enumerate(gen):
        ...
        output, main_loss, struc_aux_loss = model(data, struc_data, target) # FORWARD PASS

        loss = main_loss + struc_aux_loss * struc_aux_loss_wt
    ...
    del output, main_loss, struc_aux_loss, data, target
