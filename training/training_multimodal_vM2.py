...
def train_epochs(args, model, optimizer, params, dicts, struc_feats, struc_labels):
    ...
    for epoch in range(args.n_epochs):
        ...
        metrics_all = one_epoch(model, optimizer, epoch, args.n_epochs, args.batch_size, args.data_path, test_only, dicts, model_dir, 
                                args.gpu, args.quiet, X_train_std, X_val_std, X_test_std, y_train, args.train_frac, args.test_frac, opt_thresh)
        end = time.time()
...
def one_epoch(model, optimizer, epoch, n_epochs, batch_size, data_path, testing_only, dicts, model_dir, gpu, quiet, 
              struc_feats_train_std, struc_feats_val_std, struc_feats_test_std, struc_labels_train, train_frac, test_frac, opt_thresh):
    """
...
def train(model, optimizer, epoch, batch_size, data_path, struc_feats, struc_labels, gpu, dicts, quiet, train_frac=1): ### struc feats = full structured sparse matrix
    """
        Training loop.
