...
def build_matrix(ind2w, wv):
    ...
    words = [PAD_CHAR]
    W[0][:] = np.zeros(len(wv.word_vec(wv.index2word[0]))) # Should already be 0s ?
    for idx, word in tqdm(ind2w.items()): # CHANGED iteritems --> items
        if idx >= W.shape[0]:
            break
...
def load_embeddings(embed_file):
    ...
    with open(embed_file) as ef:
        for line in ef:
            ...
            vec = np.array(line[1:]).astype(np.float) # WORD AT INDEX 0
